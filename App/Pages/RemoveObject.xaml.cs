...
{
    internal partial class RemoveObject : Page, INotifyPropertyChanged
    {
        public static RemoveObject Inst { get; private set; }
        ...
        public string StatusText => _resourceLoader.GetString(Main.STATUSES[_status]);
        ...
        {
            MainWindow.Inst.StatusBarLoading(Utils.Any(_status, Main.StatusType.Loading, Main.StatusType.Processing));
            MainWindow.Inst.StatusBarSetText(_resourceLoader.GetString(Main.STATUSES[_status]));
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            FileItems.Clear();
            FileItems.AddRange(Main.Inst.FileItems);

            if (e.Parameter is ThumbnailItem thumbnailItem)
                SelectIndex(FileItems.IndexOf(thumbnailItem));

            Status = Main.StatusType.Loaded;
        }

        private async void RefreshPreviewBox()
        {
        ...
        public void SelectItem(ThumbnailItem item)
        {
            ...
            Status = Main.StatusType.Loading;
            ...
            item.LoadImageCacheIfNotExist(true, new Progress<int>((int cacheImageLevel) =>
            {
                ...
                else
                {
                    ...
                    CurrentItem = null;
                    Status = Main.StatusType.LoadFailed;
                }
            }));
        }
        ...
        private void InputFilesButton_Click(object sender, RoutedEventArgs e)
        {
            Main.Inst.OpenInputPicker(
                () => { Status = Main.StatusType.Loading; },
                (Main.StatusType status) => { Status = status; },
                (List<ThumbnailItem> items) => { FileItems.AddRange(items); }
            );
        }
        ...
        private void ControlButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (Utils.Any(_status, Main.StatusType.Loading, Main.StatusType.Processing)) return;
        ...
        private void InpaintImage(Windows.Foundation.Point endPoint)
        {
            ...
            ClassifyImage.LoadTensorFlow(_currentItem);
        ...
        private async void Input_Drop(object sender, DragEventArgs e)
        {
            ...
            if (e.DataView.Contains(StandardDataFormats.StorageItems))
            {
                ...
                if (storageItems.Count > 0)
                    Main.Inst.AddFiles(storageItems.Select(i => i.Path).ToList());
            }
        ...
        {
            var processing = Utils.Any(_status, Main.StatusType.Loading, Main.StatusType.Processing);
        ...
        {
            MainWindow.Inst.SetCurrentNavigationViewItem(typeof(Main).ToString());
        }
    }
}